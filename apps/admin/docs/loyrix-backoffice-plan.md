...
| [3](#phase-3--privydock-read-only)    | PrivyDock, read-only   | 15 / 15     | Done                 |
...
- [x] **Capture on console open**, not on a schedule — Vercel Cron is paid on this account. `after()` runs it once the response is sent, so it adds no page latency. First run backfills everything Cloudflare retains; later runs top up 5 days and are skipped if one ran within 6 hours, guarded first by an in-process timestamp so Next's route prefetching cannot turn one visit into several database reads.
- [x] **Capture now** button for an explicit run, with a pending state — the round trip takes seconds, so the control has to say so.
- [x] Backfill — **done 2026-08-13: 834 rows, 91 days, 2026-05-15 → 2026-08-13**. July is now permanent.
...
| 2026-08-13 | **Phase 3 complete.** Capture is triggered by opening the PrivyDock console rather than by a scheduler, via `after()` so it costs no page latency, with a 6-hour freshness guard so repeat visits do not re-run it. Backfill captured 834 rows over 91 days (2026-05-15 → 2026-08-13); totals cross-check against direct queries — 40 DMG downloads, 20 appcast polls. Cloudflare can now roll its window without losing anything.                                                                                                                                      |
