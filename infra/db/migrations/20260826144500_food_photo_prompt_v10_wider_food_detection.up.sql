-- 20260826144500_food_photo_prompt_v10_wider_food_detection.up.sql
--
-- Adds v10 of the three food-photo prompts: v9 with a loosened food/no-food
-- gate. Inserted as published but NOT active, so applying this migration
-- changes nothing until an operator activates it from the admin Prompts page.
--
-- Why: v9 folded two different questions into one. "Is anything edible here?"
-- and "what exactly is it?" were governed by the same conservative rules, so a
-- photo the model found hard to read came back as "No food detected" rather
-- than as a low-confidence guess. A real user photographed a plate of roasted
-- chana, nuts, seeds and dates twice, retried five times, was rejected every
-- time, and tripped the daily non-food limit — locked out of scanning for a
-- day over food that was plainly there.
--
-- v10 splits the two decisions. The presence question is now generous and
-- names the unplated cases that were being thrown away (loose nuts, seeds,
-- dried fruit, a pile on a table, food on a weighing scale or in a hand), and
-- says outright that being unsure WHICH food is present never justifies
-- reporting that NO food is present. The identification question keeps every
-- v9 guard: no invented items, no hidden ingredients, no assumed condiments,
-- low confidence and aliases when uncertain.
--
-- Only the VISIBLE-ONLY RULES block changed, and it changed identically in all
-- three prompts. Every nutrition, portion, cooking-method, output-mapping and
-- advice-safety section is unchanged from v9, byte for byte.
...
insert into ai_prompt_versions (
  ...
  (
    ...
    'gemini',
    'Global food photo analysis, wider food detection',
    $prompt$
...
large.
  ...
  (
    ...
    'gemini',
    'India food photo analysis, wider food detection',
    $prompt$
...
large.
  ...
  (
    ...
    'gemini',
    'Global (non-India) food photo analysis, cooking method',
    $prompt$
You are LogMyPlate's advanced global food recognition and nutrition analysis AI. Analyze the attached
meal photo for an editable food journal. Be cuisine-neutral and globally aware: recognize common
