-- 20260808175309_food_photo_prompt_v9_cooking_method.up.sql
--
-- Adds v9 of the three food-photo prompts: v8 plus a cookingMethod field.
...
--
-- Built on v8, not on the currently live v7. v8 was applied earlier but never
-- activated, and it carries the sharper advice wording; branching from v7 would
-- silently drop that work.
--
-- cookingMethod feeds the Part B / Part C cooking modifier (fried, sauced_creamy,
-- baked, grilled, steamed, raw). It is deliberately absent from the response
-- schema's required list, and the prompt tells the model to omit it when the
-- technique is not visually clear: a confident wrong guess moves the score,
-- whereas an omission simply skips the modifier.
--
-- The existing `preparation` field is untouched. It describes provenance
-- (home/restaurant/packaged), which is a different question.
--
-- Everything else — the profile block, the advice safety rules, and every
-- nutrition and portion section — is unchanged from v8, byte for byte.
...
insert into ai_prompt_versions (
  ...
  (
    ...
    'gemini',
    'Global food photo analysis, cooking method',
    $prompt$
...
large.
  ...
  (
    ...
    'gemini',
    'India food photo analysis, cooking method',
    $prompt$
